...
# Team routes must stay before /{user_id}; otherwise "teams" can be parsed as user_id.
...
async def delete_user(
    ...
    event_count = await db.scalar(
        select(func.count()).select_from(DowntimeEvent).where(
            or_(
                DowntimeEvent.opened_by_user_id == user_id,
                DowntimeEvent.closed_by_user_id == user_id,
            )
        )
    )
    if event_count:
